...
Payment events → Kafka → Python consumer (Faust)
                         ...
                         (outbox)      (graph)      (ZSET)
                              ...
                              Risk flag aggregator
                                          ↓
                              Explanation service (evidence → Ollama / Claude / rule-based)
                                          ↓
                              FastAPI  /api  +  /api/pipeline
                                          ↓
                              React app (live API, or a bundled snapshot for the demo)
```
...
- **GNN layer — features, training, evaluation, inference, ensembling.** This is
  ...
  *Artifacts & repo hygiene*); full numbers and methodology in `ml/RESULTS.md`.
...
- A standalone, FastAPI-served viewer at `/viz` (`app/viz/`) that surfaces the
  ...
  table. This is where the GNN's per-account scores are written back (Neo4j
  `gnn_risk_score`) and aggregated into `risk_flags` (type `AGGREGATE`).
  Its JSON routes are also mounted at `/api/pipeline/*` so the React app has one
  base URL; the React **Pipeline** view is the maintained surface and this page is
  kept as the no-build fallback. Design + plans in `docs/superpowers/`.

**Query + showcase API (`app/api/`) — built:**
- `/api/stats/overview` and `/volume-series` (whole-graph counts and per-currency
  cumulative volume, served from an in-process `StatsCache` warmed at startup —
  one ~8 s TRANSFER pass, then in-memory), `/api/alerts` (+ `PATCH .../status`),
  `/api/transactions`, `/api/graph/{subgraph,shortest-path,flow,account/{id}}`,
  `/api/accounts/{id}/enrich`, `/api/system/llm`, `/api/datasets/*`.
- Time-windowed reads anchor at `dataset.activity_end_ts` (the 99.9th-percentile
  transaction timestamp), **not** wall-clock and not the literal last edge: this
  dataset's activity runs Sep 2–11 2022 and then trails off to ~400 rows over the
  final week, so a 7-day window ending at the last timestamp held ~300 rows.

**Explanation layer (`app/services/explanation_service.py`) — built:**
- Replaces the old stub and the deleted `app/agent/` (which used 3.10-only syntax
  on a 3.9 runtime and matched a `business_id` only the mock seed accounts had).
  Evidence is gathered from the stores by a pure `build_evidence`, then a provider
  returns schema-validated JSON: `ollama` (local, default when reachable),
  `anthropic` (opt-in via `ANTHROPIC_API_KEY`), or `none` (deterministic text from
  the signals). Any provider error or off-schema response falls back to `none`, so
  the endpoint never fails for lack of a model.

**Frontend (`Frontend/`) — built and wired:**
- React 19 + Vite + Tailwind 4 + Cytoscape. Five views (Overview · Graph · Alerts ·
  Transactions · Pipeline) plus Upload in live mode, all on real data.
- **Two modes behind one interface** (`src/services/dataSource.js`): a live API
  source, or a committed snapshot in `src/data/snapshot/` (generated by
  `scripts/export_snapshot.py`, ≤ 4 MB) that makes the whole app work with no
  backend — that is what gets deployed to Vercel. Views never branch on the mode;
  calls the snapshot can't serve reject with `NotInSnapshotError` and render as an
  inline note.

**Dataset upload ("Analyze a file") — built:**
- `POST /api/datasets/upload` validates the CSV with the ingestor's own row parser
  *before* touching anything, then runs one job through `pipeline_runs`:
  reset → ingest → pagerank → louvain → cycle → gnn → aggregate. Destructive by
  design (it replaces the loaded graph), so it is live-mode only and gated on an
  explicit `confirm=replace`. The GNN stage runs with `feature_source="live"`,
  rebuilding the 47-column matrix from the stores — the cached IBM `.npz` is
  meaningless for someone else's graph.

**Not started:**
- Time-bounded GNN features (limitation #2 in the ML section below)
- A hosted public backend (the demo is snapshot-backed by design — see the README)
...
| API | FastAPI |
| Frontend | React 19 + Vite + Tailwind 4 + Cytoscape.js |
| Explanations | Ollama (local, default) · Claude API (opt-in) · rule-based fallback |
...
## Service module map (`app/`)

| module | role |
|---|---|
| `app/api/main.py` | the FastAPI app: CORS (loopback regex + allow-list), routers, startup cache warm-up |
| `app/api/endpoints.py` | every `/api` route |
| `app/services/stats_service.py` | `StatsCache` — one TRANSFER histogram pass; pure series math (anchored periods, cumulative volume, uniform-pace baseline) |
| `app/services/alerts_service.py` | `risk_flags` rows → alerts; status updates |
| `app/services/transactions_service.py` | `TRANSFER` reads; tier = max(sender, receiver); flagged join |
| `app/services/graph_service.py` | subgraph / shortest-path / flow / account, and the one `node_data` shaping rule |
| `app/services/explanation_service.py` | evidence → provider (ollama / anthropic / none) → validated explanation |
| `app/services/dataset_validation.py` | upload pre-flight, using the ingestor's own row parser |
| `app/services/dataset_runner.py` | the replace-and-run upload job |
| `app/viz/runner.py` | `PipelineRunner` — pagerank → louvain → cycle → gnn → aggregate; `feature_source="live"` for uploads |
| `app/viz/store.py`, `metrics.py`, `threshold.py`, `truth.py` | pipeline reads, whole-graph confusion cache, tuned cutoff, ground-truth labels |
| `scripts/export_snapshot.py` | the demo bundle (≤ 4 MB, committed to the frontend) |
...
- **`ml/runs/` and `ml/cache/` are gitignored and no longer tracked** — trained
  checkpoints and feature caches are regenerable, not source. Get them with
  `python3 scripts/fetch_models.py` (a GitHub Release asset, ~52 MB) or reproduce
  the champion with the training command above.
- **Known debt: the six `HI-Small_Trans.csv.part-*` chunks (~560 MB) are still
  tracked**, despite `Backend/benchmarks/data/` being in `.gitignore` — they were
  force-added before the rule existed, so the ignore does not apply. Every clone
  pays for them. Removing them properly means rewriting history (or at least
  `git rm --cached` plus a fresh release asset), which is a deliberate, separate
  change — not something to fold into a feature branch.
- `Frontend/src/data/snapshot/` **is** committed on purpose: it is the deployed
  demo's dataset, ~1 MB, regenerated by `scripts/export_snapshot.py`.
...
**AI enrichment** (`app/services/explanation_service.py`) — gather the account's evidence (GNN score, cycle membership, community, PageRank, top payers/payees, open flags) into a pure `AccountEvidence`, hand it to a provider, get back schema-validated `{risk_level, confidence, explanation, detected_typology, compliance_summary}`. Explainability is a regulatory requirement, not optional — so the rule-based provider is a real fallback, not a placeholder, and every path returns a written reason. The evidence prompt deliberately carries **no ground-truth label or typology**: the model must reason from structure and signals. `RiskAggregator` delegates its low-confidence band (GNN score within ±0.15 of the 0.45 threshold) here.
...
# Key queries (built — `app/services/graph_service.py`)

- `GET /api/graph/shortest-path` — how two accounts are connected through intermediaries
- `GET /api/graph/subgraph?account_id=&depth=` — neighborhood up to N hops (APOC, with a non-APOC fallback)
- `GET /api/graph/flow?...&window=` — volume and count in a window, read from `TRANSFER` edges
- `GET /api/graph/account/{id}` — one account's node data

`NodeData` carries both the aggregator verdict (`risk_score`/`risk_tier`, written
only once the aggregator has run) **and** the model's own `gnn_risk_score`/
`gnn_risk_tier`, plus `in_cycle` and `marked`. Real accounts only have the `gnn_*`
properties — reading `risk_score` alone renders every account "low", which is
