...
- Dev graph visualiser (`tools/visualize_neo4j.py`) — self-contained HTML force-directed view of FLOWS_TO, coloured by `community_id`. `--community <id>` isolates one; `--highlight <id>[,<id>...]` rings specific communities without hiding the rest, for comparing a flagged cluster against its neighbours.
...
python -m tools.visualize_neo4j --open     # HTML graph of communities; --community <id>, --highlight <id>[,<id>...], --prefix DEMO_LV_, --all
...
**Louvain community detection** (`fraud/community_detector.py`) — daily batch over aggregate `FLOWS_TO` edges active in the last `LOUVAIN_WINDOW_DAYS` (default 30). Catches the shapes cycle detection is blind to: gather-scatter, scatter-gather, fan-in/out, bipartite, stacks. Partitioning runs Python-side behind `LOUVAIN_ENGINE`: seeded `networkx.community.louvain_communities` by default, or `leidenalg`/`igraph` Leiden as opt-in (internally-connected communities by construction, scales better; GPL). Edge weight `log1p(total_amount)` (`LOUVAIN_WEIGHT_MODE`) — value-aware but whale-dampened. Every community is split into connected components before scoring (fixes Louvain's disconnected-community defect). Scored on five dimensions — size band (0.10), density as `tree_excess_ratio`: edges beyond the bare spanning-tree minimum (n−1) needed to stay connected, relative to that minimum (0.15; scale-invariant — a bare tree scores exactly 0 at any size, unlike the original edges/possible-pairs fraction, which couldn't tell a mesh from a chain once a community got past a few dozen members), internal volume (0.25), isolation = 1−conductance (0.15; computed directly from precomputed weighted node degree rather than `nx.conductance` per community — same result, dramatically fewer node visits), and overlap with accounts flagged by *other* detectors (0.35; excludes its own flags to avoid a feedback loop). Communities ≥ medium persist to `risk_flags` as `flag_type='COMMUNITY'`; fingerprint = sha256 of top-K weighted-degree core (stable under peripheral churn; core split/merge spawns a new flag — accepted blindspot). All kept communities get `community_id` node properties. Scheduling is a deploy concern (cron) — not wired in-process.
...
- **Louvain** (`louvain_runner.py`, non-cycle typologies): **81.65% recall overall** (scatter-gather 100%, gather-scatter/fan-in/fan-out 95–98%, random 90%, bipartite/stack the persistent weak spots at 42–47%), **precision 1.23%** (17 TP / 1,364 FP, `--with-postgres`, cross-detector overlap from cycle detection's flags) — up from an untuned 0.7% via a density-formula fix (see `tree_excess_ratio` above). Two other fixes were tried and reverted after verification against real labeled data, not left half-applied:
  - An edge-weight filter (drop account pairs below a minimum combined `tx_count` before Louvain sees them) traded recall for precision (84% precision / 32.6% recall at the tuned threshold) but couldn't actually separate signal from noise — checked directly against a labeled 18-account gather-scatter ring: 21/33 of its real internal edges had `tx_count=1`, same as background noise. Dollar amount and time-span were checked too; neither separates real rings from noise at the edge level in this dataset either.
  - Gating the isolation/cohesion score to 0 for communities that equal their entire weakly-connected component (99.7% of components in a typical window have < 20 nodes, so "isolation" is structurally guaranteed rather than chosen for almost everything) sounded right, but removed the *only* signal keeping every one of the un-gated formula's true positives above the medium threshold — precision went to 0.00%, not up. Small real rings need the same "trivial" isolation credit noise gets.

  Batch runtime: **32.5s** for the full benchmark (was 2,050s+ before vectorizing `community_conductance` off `nx.conductance` — same output, O(members) instead of nx's cost per community). A ~1–2% precision ceiling on pure topology signals looks real for this benchmark; cross-detector corroboration is the strongest lever found so far. No external baseline exists for these typologies on HI-Small; this runner is the reference number.
...
- **Louvain follow-ups** (see `docs/superpowers/plans/2026-07-03-louvain-clustering.md` for the full original list): cron wiring, Jaccard flag-matching if core churn duplicates alerts, server-side `gds.leiden` if graph-in-memory becomes the ceiling, AMLGentex as an external scenario generator. Knob tuning against the baseline (see Benchmarks) is done for this round and hit a real ceiling — a simple time-span check (total span of a labeled ring's edges vs. noise clusters') didn't separate them either, so temporal features aren't an obviously free win. Precision meaningfully past ~1-2% standalone likely needs a different data signal (account metadata, deeper cross-detector corroboration) rather than more topology tuning — matches the broader plan of Louvain as a GNN feature/weak-label source rather than a standalone alerting system.
